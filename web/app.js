...
(function () {
  ...
  function renderPromptLibrary() {
    ...
    var categories = Array.from(new Set(['通用'].concat(library.categories || [], prompts.map(function (prompt) { return (prompt.category || '通用').trim() || '通用'; })))).sort(function (left, right) { if (left === '通用') return -1; if (right === '通用') return 1; return left.localeCompare(right, 'zh-CN'); });
    var visible = prompts;
  ...
  function newPrompt(targetCategory) { var category = targetCategory || (state.promptCategoryFilter === 'all' ? '通用' : state.promptCategoryFilter); state.promptCategoryFilter = category; state.promptCollapsedCategories[category] = false; api('/api/prompt-library', { method: 'POST', body: JSON.stringify({ action: 'create', category: category }) }).then(function (res) { if (!res.ok) { toast(res.error || '新建失败'); return; } state.promptLibrary = res.promptLibrary; state.promptTrashOpen = false; state.promptId = res.promptLibrary.prompts[0].id; renderPromptLibrary(); $('#promptTitle').focus(); }); }
  ...
  function deletePromptCategory(category) {
    ...
    if (!confirm('确定删除“' + category + '”文件夹吗？其中的 ' + count + ' 条提示词会移入“通用”，内容不会删除。')) return;
    api('/api/prompt-library', { method: 'POST', body: JSON.stringify({ action: 'delete-category', category: category, currentPrompt: state.promptId ? readPromptPayload() : null }) }).then(function (res) {
      if (!res.ok) { toast(res.error || '删除文件夹失败'); return; }
      state.promptLibrary = res.promptLibrary;
      if (state.promptCategoryFilter === category) state.promptCategoryFilter = 'all';
      delete state.promptCollapsedCategories[category];
      renderPromptLibrary();
      toast('文件夹已删除，提示词已移入“通用”');
