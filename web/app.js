...
(function () {
  ...
  function renderJournalTracker() {
    ...
    function renderArticleGroup(key, label, items) {
      ...
      var pager = pageCount > 1 ? '<nav class="tracker-article-pager" aria-label="' + label + '分页"><button type="button" data-tracker-page="' + key + '" data-tracker-page-delta="-1"' + (currentPage <= 1 ? ' disabled' : '') + '>上一页</button><span>第 ' + currentPage + ' / ' + pageCount + ' 页 · ' + (startIndex + 1) + '–' + Math.min(startIndex + pageSize, items.length) + ' / ' + items.length + ' 篇</span><button type="button" data-tracker-page="' + key + '" data-tracker-page-delta="1"' + (currentPage >= pageCount ? ' disabled' : '') + '>下一页</button></nav>' : '';
