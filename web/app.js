...
(function () {
  ...
  function restoreTrackerCategoryCatalog() {
    try {
      ...
      state.trackerCustomCategories = Array.isArray(saved) ? Array.from(new Set(saved.map(cleanTrackerCategoryName).filter(function (name) { return name && name !== '未分类' && name !== '__default__' && name !== '__new_category__'; }))) : [];
    } catch (_) { state.trackerCustomCategories = []; }
  }
  function saveTrackerCategoryCatalog() {
    state.trackerCustomCategories = Array.from(new Set((state.trackerCustomCategories || []).map(cleanTrackerCategoryName).filter(function (name) { return name && name !== '未分类' && name !== '__default__' && name !== '__new_category__'; })));
    try { localStorage.setItem(trackerCategoryCatalogKey, JSON.stringify(state.trackerCustomCategories)); } catch (_) {}
  }
  function rememberTrackerCategory(category) {
    var name = cleanTrackerCategoryName(category);
  ...
  function updateTrackerCategoryCatalog(action) {
    ...
    if (action.action === 'rename-category') {
      ...
      if (Object.prototype.hasOwnProperty.call(state.trackerJournalCategoryCollapsed, action.from)) {
        ...
        delete state.trackerJournalCategoryCollapsed[action.from];
      ...
      try { localStorage.setItem('academic-workbench-journal-categories-collapsed-v1', JSON.stringify(state.trackerJournalCategoryCollapsed)); } catch (_) {}
      saveTrackerDisplayPreferences();
    } else if ((action.action === 'set-category' || action.action === 'set-categories') && action.category) {
      rememberTrackerCategory(action.category);
    }
  }
  ...
  function saveTrackerJournalCategory(id, category) {
    journalTrackerRequest({ action: 'set-category', id: id, category: category }).then(function (result) {
      updateTrackerCategoryCatalog({ action: 'set-category', category: category });
      applyJournalTrackerData(result);
  ...
  function runTrackerCategoryAction(payload, successMessage) {
    journalTrackerRequest(payload).then(function (result) {
      ...
      updateTrackerCategoryCatalog(payload);
      applyJournalTrackerData(result);
  ...
  function saveTrackedArticleRead(id, isRead) {
    return journalTrackerRequest({ action: 'set-read', id: id, isRead: isRead }).then(function (result) {
      ...
      if (isRead) { state.trackerCollapsedGroups.read = true; try { localStorage.setItem('academic-workbench-tracker-collapsed-v1', JSON.stringify(state.trackerCollapsedGroups)); } catch (_) {} saveTrackerDisplayPreferences(); }
      applyJournalTrackerData(result);
      return result;
    });
  }
  ...
  function markAllTrackedArticlesRead(button) {
    ...
    var batches = [];
    for (var offset = 0; offset < selectedIds.length; offset += 300) batches.push(selectedIds.slice(offset, offset + 300));
    var latestResult = null;
    var updatedTotal = 0;
    var markPromise = Promise.resolve();
    batches.forEach(function (batch) {
      markPromise = markPromise.then(function () {
        return journalTrackerRequest({ action: 'mark-selected-read', ids: batch }).then(function (result) {
          var updatedArticles = result.articles || [];
          var selectedStillUnread = updatedArticles.some(function (article) { return batch.indexOf(String(article.id)) >= 0 && article.is_read !== true; });
          if (result.readStateVersion !== 1 || selectedStillUnread) throw new Error('部分文章未能标记为已读，请刷新后重试。');
          latestResult = result;
    ...
    markPromise.then(function () {
      ...
      saveTrackerDisplayPreferences();
      if (latestResult) applyJournalTrackerData(latestResult);
      toast('已将 ' + updatedTotal + ' 篇文章标记为已读');
    }).catch(function (error) {
      if (latestResult) applyJournalTrackerData(latestResult);
  ...
  function bindEvents() {
    ...
    $('#trackerSubscriptions').addEventListener('click', function (event) {
      var categoryToggle = event.target.closest('[data-tracker-category-toggle]');
      if (categoryToggle) {
        var category = categoryToggle.dataset.trackerCategoryToggle;
        state.trackerJournalCategoryCollapsed[category] = !state.trackerJournalCategoryCollapsed[category];
        try { localStorage.setItem('academic-workbench-journal-categories-collapsed-v1', JSON.stringify(state.trackerJournalCategoryCollapsed)); } catch (_) {}
        renderJournalTracker();
        return;
      }
      ...
      if (selectButton) {
        ...
        saveTrackerDisplayPreferences();
        renderJournalTracker();
        $('#trackerArticles').scrollIntoView({ behavior: 'smooth', block: 'start' });
        return;
      }
      var saveButton = event.target.closest('[data-tracker-feed-save]');
      if (saveButton) { saveTrackerFeed(saveButton.dataset.trackerFeedSave, saveButton); return; }
    ...
    $('#trackerArticles').addEventListener('click', function (event) {
      ...
      var pageButton = event.target.closest('[data-tracker-page]');
      if (pageButton) {
      ...
      var groupButton = event.target.closest('[data-tracker-group-toggle]');
      if (groupButton) {
        var group = groupButton.dataset.trackerGroupToggle;
        state.trackerCollapsedGroups[group] = !state.trackerCollapsedGroups[group];
        try { localStorage.setItem('academic-workbench-tracker-collapsed-v1', JSON.stringify(state.trackerCollapsedGroups)); } catch (_) {}
        saveTrackerDisplayPreferences();
        renderJournalTracker();
        return;
      ...
      var rankButton = event.target.closest('[data-tracker-journal-rank]');
      if (rankButton) { queryTrackerJournalRank(rankButton.dataset.trackerJournalRank, rankButton); return; }
      if (event.target.closest('[data-tracker-open-detail]')) { openTrackerArticleDetail(event.target.closest('[data-tracker-open-detail]').dataset.trackerOpenDetail); return; }
    ...
    $('#promptList').addEventListener('input', function (e) {
      if (e.target.id !== 'promptSearch') return;
      var start = e.target.selectionStart, end = e.target.selectionEnd;
      state.promptQuery = e.target.value;
      renderPromptFolderTree();
      var search = $('#promptSearch');
      if (search) { search.focus(); search.setSelectionRange(start, end); }
    });
    $('#promptList').addEventListener('keydown', function (event) {
      if (event.target.id !== 'promptSearch' || event.key !== 'Escape' || !state.promptQuery) return;
      event.preventDefault();
      state.promptQuery = '';
      renderPromptFolderTree();
      var search = $('#promptSearch');
    ...
    document.addEventListener('click', function (event) {
      var back = event.target.closest('[data-linked-project]');
      if (back) { openLinkedProject(back.dataset.linkedProject); return; }
      var remove = event.target.closest('[data-project-link-remove]');
      if (remove) { removeProjectLink(remove.dataset.projectLinkRemove, remove.dataset.linkId); return; }
      var open = event.target.closest('[data-project-link-open]');
      if (open) openProjectLinkedRecord(open.dataset.projectLinkOpen, open.dataset.linkId);
    });
    ...
    $('#variableImportUndo').addEventListener('click', openVariableImportHistory);
    $('#variableImportHistoryClose').addEventListener('click', closeVariableImportHistory);
    $('#variableImportHistoryBackdrop').addEventListener('click', closeVariableImportHistory);
    $('#variableImportHistoryList').addEventListener('click', function (event) {
    ...
    $('#variableEditor').addEventListener('dblclick', function (event) {
      ...
      var roles = active ? normalizeVariableMeasureReferences(active)[0].role : null;
    ...
    $('#refFolders').addEventListener('click', function (event) {
      ...
      var remove = event.target.closest('[data-ref-folder-delete]');
      if (remove) { deleteReferenceFolder(remove.dataset.refFolderDelete); return; }
    ...
    $('#refQualityAudit').addEventListener('click', function (event) {
      ...
      if (String(id) === String(state.referenceId)) return;
      afterCurrentEditorSaved('references', function () { state.referenceId = id; renderReferenceLibrary(); });
    });
    ...
    $('#refList').addEventListener('change', function (event) {
    ...
    window.addEventListener('message', function (event) {
      ...
      if (event.data.type === 'academic-research-hub-request-project-links' && event.source === $('.research-hub-frame').contentWindow) {
        api('/api/research-projects').then(function (res) {
          if (!res.ok) return;
          state.researchProjects = res.researchProjects || { projects: [], trash: [] };
          publishPaperProjectLinks();
        }).catch(function () {});
      }
  ...
  function loadReferenceLibrary() {
    return Promise.all([api('/api/references'), api('/api/research-projects'), api('/api/knowledge-base')]).then(function (results) {
      ...
      state.referenceLibrary = Object.assign({ items: [], trash: [], folders: [] }, library.referenceLibrary || {});
  ...
  function referenceFields() { return ['refTitle', 'refAuthors', 'refYear', 'refType', 'refSource', 'refLocator', 'refTags', 'refDoi', 'refUrl', 'refFolder', 'refAbstract', 'refKeywords', 'refProject', 'refKnowledge', 'refNotes']; }
  ...
  function renderReferenceFolders() {
    var library = state.referenceLibrary || {}, folders = Array.isArray(library.folders) ? library.folders : [], items = library.items || [], root = $('#refFolders');
    if (!folders.some(function (folder) { return String(folder.id) === String(state.referenceFolderId); }) && ['all', 'unfiled'].indexOf(state.referenceFolderId) < 0) state.referenceFolderId = 'all';
    function folderRow(id, label, count, canManage) {
      var selected = String(state.referenceFolderId) === String(id);
      return '<div class="ref-folder-row' + (selected ? ' is-active' : '') + '"><button type="button" class="ref-folder-select" data-ref-folder-select="' + escapeHtml(String(id)) + '" title="' + escapeHtml(label) + '"><span class="ref-folder-glyph" aria-hidden="true">▰</span><span class="ref-folder-name">' + escapeHtml(label) + '</span><span class="ref-folder-count">' + count + '</span></button>' + (canManage ? '<div class="ref-folder-tools"><button type="button" data-ref-folder-rename="' + escapeHtml(String(id)) + '" title="重命名文件夹" aria-label="重命名' + escapeHtml(label) + '">✎</button><button type="button" data-ref-folder-delete="' + escapeHtml(String(id)) + '" title="删除文件夹" aria-label="删除' + escapeHtml(label) + '">×</button></div>' : '') + '</div>';
    }
    var unfiledCount = items.filter(function (item) { return !item.mergedInto && !String(item.folderId || ''); }).length;
    root.innerHTML = folderRow('all', '全部文献', items.filter(function (item) { return !item.mergedInto; }).length, false) + folderRow('unfiled', '未分类', unfiledCount, false) + folders.map(function (folder) {
      var count = items.filter(function (item) { return !item.mergedInto && String(item.folderId || '') === String(folder.id); }).length;
  ...
  function renameReferenceFolder(id) {
    ...
    if (!folder) return;
    var name = prompt('重命名文件夹：', folder.name || '');
    if (!name || !name.trim() || name.trim() === folder.name) return;
    afterCurrentEditorSaved('references', function () { return api('/api/references', { method: 'POST', body: JSON.stringify({ action: 'folder-rename', id: id, name: name.trim() }) }).then(function (res) {
      if (!res.ok) { toast(res.error || '重命名失败'); return; }
      state.referenceLibrary = res.referenceLibrary; renderReferenceLibrary(); toast('文件夹已重命名');
    }).catch(function (error) { toast(error.message || '重命名失败'); }); });
  }
  function deleteReferenceFolder(id) {
    var folder = ((state.referenceLibrary || {}).folders || []).find(function (entry) { return String(entry.id) === String(id); });
    if (!folder || !confirm('删除文件夹“' + (folder.name || '未命名文件夹') + '”？其中的文献会保留并移入“未分类”。')) return;
    afterCurrentEditorSaved('references', function () { return api('/api/references', { method: 'POST', body: JSON.stringify({ action: 'folder-delete', id: id }) }).then(function (res) {
      if (!res.ok) { toast(res.error || '删除文件夹失败'); return; }
      state.referenceLibrary = res.referenceLibrary;
      if (String(state.referenceFolderId) === String(id)) { state.referenceFolderId = 'unfiled'; state.referenceId = null; }
      renderReferenceLibrary(); toast('文件夹已删除，文献已保留在未分类');
    }).catch(function (error) { toast(error.message || '删除文件夹失败'); }); });
  }
  function renderReferenceLibrary() {
    var library = state.referenceLibrary || { items: [], trash: [], folders: [] }, items = library.items || [], trash = library.trash || [], list = $('#refList');
    library.folders = Array.isArray(library.folders) ? library.folders : [];
    ...
    $('#refFolderNew').disabled = false; $('#refFolderNew').hidden = false;
    ...
    var active = activeReference();
    list.innerHTML = '<div class="ref-list-label">我的文献 <span>' + visible.length + '/' + items.length + '</span></div>' + (visible.length ? visible.map(function (item) {
      var mergedTarget = item.mergedInto ? (items.filter(function (entry) { return String(entry.id) === String(item.mergedInto); })[0] || {}).title : '';
      var folderOptions = '<option value=""' + (!String(item.folderId || '') ? ' selected' : '') + '>未分类</option>' + (library.folders || []).map(function (folder) { return '<option value="' + escapeHtml(String(folder.id)) + '"' + (String(folder.id) === String(item.folderId || '') ? ' selected' : '') + '>' + escapeHtml(folder.name || '未命名文件夹') + '</option>'; }).join('');
      return '<div class="ref-list-entry"><button type="button" draggable="true" class="ref-list-item' + (String(item.id) === String(state.referenceId) ? ' is-active' : '') + (item.mergedInto ? ' is-merged-alias' : '') + '" data-ref-id="' + escapeHtml(String(item.id)) + '" title="可拖到左侧文件夹，或使用下方菜单移动"><b>' + escapeHtml(item.title || '未命名文献') + '</b><span>' + escapeHtml(item.authors || '作者待补充') + ' · ' + escapeHtml(item.year || '年份待补充') + '</span><i>' + escapeHtml(item.mergedInto ? '已并入：' + (mergedTarget || '主条目') : (item.type || '期刊论文')) + '</i></button><label class="ref-list-move"><span>移动到</span><select data-ref-move-id="' + escapeHtml(String(item.id)) + '" aria-label="移动《' + escapeHtml(item.title || '未命名文献') + '》到文件夹">' + folderOptions + '</select></label></div>';
    }).join('') : '<div class="ref-empty">还没有匹配的文献<br>点击右上角新建或导入 BibTeX</div>');
    ...
    $('#refTitle').value = active ? active.title || '' : ''; $('#refAuthors').value = active ? active.authors || '' : ''; $('#refYear').value = active ? active.year || '' : ''; $('#refType').value = active ? active.type || '期刊论文' : '期刊论文'; $('#refSource').value = active ? active.source || '' : ''; $('#refLocator').value = active ? active.locator || '' : ''; $('#refTags').value = active ? active.tags || '' : ''; $('#refDoi').value = active ? active.doi || '' : ''; $('#refUrl').value = active ? active.url || '' : ''; $('#refFolder').value = active ? String(active.folderId || '') : ''; $('#refAbstract').value = active ? active.abstract || '' : ''; $('#refKeywords').value = active ? active.keywords || '' : ''; $('#refAbstractSource').textContent = active && active.abstractSource ? '来源：' + active.abstractSource : ''; $('#refKeywordsSource').textContent = active && active.keywordsSource ? '来源：' + active.keywordsSource : ''; $('#refProject').value = active ? String(active.projectId || '') : ''; $('#refKnowledge').value = active ? String(active.knowledgeDocId || '') : ''; $('#refNotes').value = active ? active.notes || '' : '';
    renderReferencePreview();
    renderProjectBacklinks('reference', active && active.id, $('.ref-editor'));
    if (state.referenceAuditEnabled) renderReferenceQualityAudit();
  }
  function filteredReferenceItems() {
    ...
    return items.filter(function (item) {
      ...
      var folderFilter = state.referenceFolderId || 'all';
      var folderMatches = folderFilter === 'all' || (folderFilter === 'unfiled' ? !String(item.folderId || '') : String(item.folderId || '') === String(folderFilter));
      return folderMatches && (state.referenceShowMerged || !item.mergedInto) && (!needle || content.indexOf(needle) >= 0) && (state.referenceTypeFilter === 'all' || item.type === state.referenceTypeFilter);
    });
  }
  ...
  function normalizeReferenceTitle(value) {
    return String(value || '').toLocaleLowerCase().replace(/[\s\p{P}\p{S}]+/gu, '').trim();
  }
  function findReferenceDuplicates(items) {
    var parent = items.map(function (_, index) { return index; });
    function root(index) { while (parent[index] !== index) { parent[index] = parent[parent[index]]; index = parent[index]; } return index; }
    function join(left, right) { var a = root(left), b = root(right); if (a !== b) parent[b] = a; }
    var buckets = Object.create(null);
    items.forEach(function (item, index) {
      var doi = normalizeReferenceDoi(item.doi), title = normalizeReferenceTitle(item.title), year = String(item.year || '').replace(/\D/g, '').slice(0, 4);
      var keys = [];
      if (doi) keys.push('doi:' + doi);
      if (title.length >= 8 && year) keys.push('title-year:' + title + ':' + year);
      keys.forEach(function (key) { if (buckets[key] != null) join(index, buckets[key]); else buckets[key] = index; });
    });
    var groups = Object.create(null);
    items.forEach(function (item, index) { var key = root(index); (groups[key] || (groups[key] = [])).push(item); });
    return Object.keys(groups).map(function (key) { return groups[key]; }).filter(function (group) { return group.length > 1; });
  }
  function missingReferenceFields(item) {
    var missing = [];
    if (!String(item.title || '').trim()) missing.push('标题');
    if (!String(item.authors || '').trim()) missing.push('作者');
    if (!String(item.year || '').trim()) missing.push('年份');
    if (['期刊论文', '书籍', '会议论文', '报告'].indexOf(item.type) >= 0 && !String(item.source || '').trim()) missing.push('期刊/出版社');
    if (item.type === '网页' && !String(item.url || '').trim()) missing.push('网页链接');
    return missing;
  }
  ...
  function readReferencePayload() { return { action: 'save', id: state.referenceId, title: $('#refTitle').value, authors: $('#refAuthors').value, year: $('#refYear').value, type: $('#refType').value, source: $('#refSource').value, locator: $('#refLocator').value, tags: $('#refTags').value, doi: $('#refDoi').value, url: $('#refUrl').value, folderId: $('#refFolder').value, abstract: $('#refAbstract').value, keywords: $('#refKeywords').value, abstractSource: $('#refAbstractSource').textContent.replace(/^来源：/, ''), keywordsSource: $('#refKeywordsSource').textContent.replace(/^来源：/, ''), projectId: $('#refProject').value, knowledgeDocId: $('#refKnowledge').value, notes: $('#refNotes').value }; }
  ...
  function queueReferenceAutoSave() { if (!state.referenceId || state.referenceTrashOpen) return; var body = readReferencePayload(); renderReferencePreview(); if (state.referenceAuditEnabled) renderReferenceQualityAudit(); queueAutoSave('reference:' + body.id, body, persistReference); }
  ...
  function newReference() { return afterCurrentEditorSaved('references', function () { var folderId = ['all', 'unfiled'].indexOf(state.referenceFolderId) >= 0 ? '' : state.referenceFolderId; return api('/api/references', { method: 'POST', body: JSON.stringify({ action: 'create', folderId: folderId }) }).then(function (res) { if (!res.ok) { toast(res.error || '新建失败'); return false; } state.referenceLibrary = res.referenceLibrary; state.referenceTrashOpen = false; state.referenceId = res.referenceLibrary.items[0].id; renderReferenceLibrary(); $('#refTitle').focus(); return true; }); }); }
  ...
  function referenceDoiProposals(reference, metadata, auxiliary) {
    auxiliary = auxiliary || {};
    ...
    if (!String(reference.abstract || '').trim() && auxiliary.abstract) { proposed.abstract = auxiliary.abstract; proposed.abstractSource = auxiliary.abstractSource || ''; }
    if (!String(reference.keywords || '').trim() && auxiliary.keywords) { proposed.keywords = auxiliary.keywords; proposed.keywordsSource = auxiliary.keywordsSource || ''; }
    return proposed;
  }
  function openAlexAbstractText(invertedIndex) {
    if (!invertedIndex || typeof invertedIndex !== 'object') return '';
    var words = [];
    Object.keys(invertedIndex).forEach(function (word) { (Array.isArray(invertedIndex[word]) ? invertedIndex[word] : []).forEach(function (position) { if (Number.isInteger(position) && position >= 0 && position < 20000) words[position] = word; }); });
    return words.filter(function (word) { return typeof word === 'string'; }).join(' ').replace(/\s+([,.;:!?])/g, '$1').trim();
  }
  async function fetchReferenceAbstractAndKeywords(doi, requester, signal) {
    var normalizedDoi = normalizeReferenceDoi(doi), semanticAbstract = '', openAlexAbstract = '', keywords = '';
    var semanticRequest = (async function () {
      try {
        var response = await requester('https://api.semanticscholar.org/graph/v1/paper/' + encodeURIComponent('DOI:' + normalizedDoi) + '?fields=abstract,externalIds', { signal: signal });
        if (!response.ok) return;
        var paper = await response.json(), reportedDoi = normalizeReferenceDoi(((paper || {}).externalIds || {}).DOI);
        if ((!reportedDoi || reportedDoi === normalizedDoi) && paper && typeof paper.abstract === 'string') semanticAbstract = paper.abstract.trim();
      } catch (_) {}
    })();
    var openAlexRequest = (async function () {
      try {
        var response = await requester('https://api.openalex.org/works/doi:' + encodeURIComponent(normalizedDoi) + '?select=doi,abstract_inverted_index,keywords', { signal: signal });
        if (!response.ok) return;
        var work = await response.json(), reportedDoi = normalizeReferenceDoi((work || {}).doi);
        if (reportedDoi && reportedDoi !== normalizedDoi) return;
        openAlexAbstract = openAlexAbstractText(work && work.abstract_inverted_index);
        keywords = (work && Array.isArray(work.keywords) ? work.keywords : []).map(function (entry) { return String((entry || {}).display_name || '').trim(); }).filter(Boolean).slice(0, 5).join('；');
      } catch (_) {}
    })();
    await Promise.all([semanticRequest, openAlexRequest]);
    return { abstract: semanticAbstract || openAlexAbstract, abstractSource: semanticAbstract ? 'Semantic Scholar' : openAlexAbstract ? 'OpenAlex（倒排索引重建）' : '', keywords: keywords, keywordsSource: keywords ? 'OpenAlex（算法主题词，并非作者原始关键词）' : '' };
  }
  ...
  function renderReferenceDoiBatchPreview(failures, stopped) {
    var panel = $('#refDoiBatchPreview'), fieldLabels = { title: '标题', authors: '作者', year: '年份', source: '期刊/出版社', locator: '卷期/页码', url: '原文链接', abstract: '摘要', keywords: '关键词' };
    var entries = state.referenceDoiBatchEntries;
    var rows = entries.map(function (entry) {
      var changes = Object.keys(entry.fields).filter(function (key) { return ['abstractSource', 'keywordsSource'].indexOf(key) < 0; }).map(function (key) { var sourceKey = key === 'abstract' ? 'abstractSource' : key === 'keywords' ? 'keywordsSource' : ''; var source = sourceKey && entry.fields[sourceKey] ? ' <small>（' + escapeHtml(entry.fields[sourceKey]) + '）</small>' : ''; var value = key === 'abstract' ? String(entry.fields[key]).slice(0, 520) + (String(entry.fields[key]).length > 520 ? '…' : '') : entry.fields[key]; return '<li><b>' + escapeHtml(fieldLabels[key] || key) + '：</b>' + escapeHtml(value) + source + '</li>'; }).join('');
  ...
  async function fetchReferenceDoiBatch() {
    ...
    var candidates = filteredReferenceItems().filter(function (item) { return !item.mergedInto && normalizeReferenceDoi(item.doi) && (!String(item.title || '').trim() || item.title === '未命名文献' || !String(item.authors || '').trim() || !String(item.year || '').trim() || !String(item.source || '').trim() || !String(item.locator || '').trim() || !String(item.url || '').trim() || !String(item.abstract || '').trim() || !String(item.keywords || '').trim()); });
    ...
    try {
      for (var index = 0; index < candidates.length; index += 1) {
        ...
        try {
          var doi = normalizeReferenceDoi(reference.doi);
          var crossrefPromise = requester('https://api.crossref.org/works/' + encodeURIComponent(doi), { signal: requestController.signal }).then(function (response) { return response.ok ? response.json() : null; }).catch(function () { return null; });
          var auxiliaryPromise = fetchReferenceAbstractAndKeywords(doi, requester, requestController.signal);
          var results = await Promise.all([crossrefPromise, auxiliaryPromise]), result = results[0], auxiliary = results[1];
          var fields = referenceDoiProposals(reference, result && result.message || {}, auxiliary);
          ...
          else failures.push((reference.title || reference.doi) + '（未找到可补充数据）');
    ...
    } finally { state.referenceDoiBatchController = null; button.disabled = false; button.textContent = '批量补全文献信息'; }
  }
  ...
  async function fetchReferenceDoi() {
    var doi = normalizeReferenceDoi($('#refDoi').value);
    if (!doi) { toast('请先输入 DOI'); return; }
    var button = $('#refFetchDoi'), requester = window.__nativeFetch || window.fetch.bind(window), controller = new AbortController();
    button.disabled = true; button.textContent = '获取中…';
    var timeoutId = setTimeout(function () { controller.abort(); }, 10000);
    try {
      var results = await Promise.all([
        requester('https://api.crossref.org/works/' + encodeURIComponent(doi), { signal: controller.signal }).then(function (res) { return res.ok ? res.json() : null; }).catch(function () { return null; }),
        fetchReferenceAbstractAndKeywords(doi, requester, controller.signal)
      ]);
      var response = results[0], metadata = response && response.message || {};
      var current = Object.assign({}, activeReference() || {}, {
        title: $('#refTitle').value, authors: $('#refAuthors').value, year: $('#refYear').value,
        source: $('#refSource').value, locator: $('#refLocator').value, url: $('#refUrl').value,
        abstract: $('#refAbstract').value, keywords: $('#refKeywords').value
      });
      var proposed = referenceDoiProposals(current, metadata, results[1]);
      if (!Object.keys(proposed).length) throw new Error('暂未从 Crossref、Semantic Scholar 或 OpenAlex 找到可补充的空白信息');
      Object.keys(proposed).forEach(function (field) {
        if (field === 'abstractSource') $('#refAbstractSource').textContent = '来源：' + proposed[field];
        else if (field === 'keywordsSource') $('#refKeywordsSource').textContent = '来源：' + proposed[field];
        else if (field === 'abstract') $('#refAbstract').value = proposed[field];
        else if (field === 'keywords') $('#refKeywords').value = proposed[field];
        else if (Object.prototype.hasOwnProperty.call({ title: 1, authors: 1, year: 1, source: 1, locator: 1, url: 1 }, field)) {
          var fieldIds = { title: 'refTitle', authors: 'refAuthors', year: 'refYear', source: 'refSource', locator: 'refLocator', url: 'refUrl' };
          if (!String($('#' + fieldIds[field]).value || '').trim() || (field === 'title' && $('#' + fieldIds[field]).value === '未命名文献')) $('#' + fieldIds[field]).value = proposed[field];
        }
      });
      queueReferenceAutoSave();
      toast(Object.keys(proposed).length ? '已补全 DOI 元数据；摘要与关键词按来源标注' : '未找到可补充的空白字段');
    } catch (error) { toast(error.name === 'AbortError' ? '查询超时，请稍后重试' : error.message || 'DOI 自动补全失败，请手动填写'); }
    finally { clearTimeout(timeoutId); button.disabled = false; button.textContent = 'DOI 自动补全'; }
  }

  // ===== 研究项目：以稳定记录 ID 关联现有内容，不复制正文 =====
  var projectLinkTypes = { paper: '论文管线', reference: '文献与引用', variable: '变量库', knowledge: '知识库', note: '公众号笔记' };
  function normalizedProjectLinks(links) {
    if (!Array.isArray(links)) return [];
    return links.filter(function (link) { return link && projectLinkTypes[link.type] && typeof link.id === 'string' && link.id; });
  }
  function publishPaperProjectLinks() {
    var frame = $('.research-hub-frame');
    if (!frame || !frame.contentWindow) return;
    frame.contentWindow.postMessage({
      type: 'academic-research-hub-project-links',
      projects: ((state.researchProjects || {}).projects || []).map(function (project) {
        return { id: project.id, title: project.title || '未命名项目', links: normalizedProjectLinks(project.links).filter(function (link) { return link.type === 'paper'; }) };
      })
    }, location.origin);
  }
  function projectPaperTargets() {
    var cards, fields;
    try { cards = JSON.parse(localStorage.getItem('research-hub-cards-v1') || '{}') || {}; fields = JSON.parse(localStorage.getItem('research-hub-fields-v1') || '{}') || {}; } catch (_) { return []; }
    var results = [];
  ...
  function projectLinkTargets(type) {
    if (type === 'paper') return projectPaperTargets();
    var source = type === 'reference' ? (state.referenceLibrary || {}).items : type === 'variable' ? (state.variableLibrary || {}).items : type === 'knowledge' ? (state.knowledgeBase || {}).docs : type === 'note' ? (state.noteStudio || {}).notes : [];
    return (source || []).map(function (item) { return { id: String(item.id), title: item.title || item.name || '未命名记录', item: item }; });
  }
  function projectLinksFor(project) {
    var links = normalizedProjectLinks(project && project.links).map(function (link) { return { type: link.type, id: link.id, implicit: false }; });
    ((state.referenceLibrary || {}).items || []).forEach(function (reference) {
      if (String(reference.projectId || '') !== String(project && project.id)) return;
      if (!links.some(function (link) { return link.type === 'reference' && link.id === String(reference.id); })) links.push({ type: 'reference', id: String(reference.id), implicit: true });
    });
    return links;
  }
  function renderProjectLinkControls(project) {
    var type = state.projectLinkType || 'paper';
    if (!projectLinkTypes[type]) type = 'paper';
    $('#projectLinkType').value = type;
    var targets = projectLinkTargets(type);
    $('#projectLinkTarget').innerHTML = '<option value="">' + (targets.length ? '选择一条记录' : '此模块暂无记录') + '</option>' + targets.map(function (target) { return '<option value="' + escapeHtml(target.id) + '">' + escapeHtml(target.title) + '</option>'; }).join('');
    $('#projectLinkType').disabled = !project;
    $('#projectLinkTarget').disabled = !project || !targets.length;
    $('#projectLinkAdd').disabled = !project || !targets.length;
    $('#projectLinkList').innerHTML = project ? (projectLinksFor(project).map(function (link) {
      var target = projectLinkTargets(link.type).find(function (entry) { return entry.id === link.id; });
      return '<div class="project-link-row"><span>' + escapeHtml(projectLinkTypes[link.type]) + '</span>' +
        (target ? '<button type="button" data-project-link-open="' + escapeHtml(link.type) + '" data-link-id="' + escapeHtml(link.id) + '" title="打开关联记录">' + escapeHtml(target.title) + '</button>' : '<em>原记录已删除</em>') +
        (link.implicit ? '<small title="在文献与引用中更改关联研究项目">文献关联</small>' : '<button type="button" class="project-link-remove" data-project-link-remove="' + escapeHtml(link.type) + '" data-link-id="' + escapeHtml(link.id) + '" aria-label="移除关联">×</button>') + '</div>';
    }).join('') || '<p class="project-link-empty">还没有关联记录；选择类型与记录后点击“关联”。</p>') : '';
  }
  function openPipelinePaper(kind, key) {
    switchPanel('research-hub');
    var frame = $('.research-hub-frame');
    if (!frame) return;
    var send = function () { frame.contentWindow.postMessage({ type: 'academic-research-hub-open-paper', kind: kind, key: key }, location.origin); };
  ...
  function openProjectLinkedRecord(type, id) {
    if (type === 'paper') {
      var paper = projectPaperTargets().find(function (entry) { return entry.id === id; });
      if (paper) openPipelinePaper(paper.kind, paper.id);
      else toast('论文已删除或尚未加载');
      return;
    }
    var target = projectLinkTargets(type).find(function (entry) { return entry.id === id; });
    if (!target) { toast('原记录已删除或尚未加载'); return; }
    var panels = { reference: 'references', variable: 'variable-library', knowledge: 'knowledge-base', note: 'note-studio' };
    openCmdkRecord(panels[type], target.item, type === 'reference' ? 'references' : type === 'variable' ? 'variables' : type === 'knowledge' ? 'knowledge' : 'notes');
  }
  function renderProjectBacklinks(type, id, host) {
    if (!host) return;
    var node = $('.project-backlinks', host);
    if (!node) { node = document.createElement('div'); node.className = 'project-backlinks'; host.appendChild(node); }
    var projects = ((state.researchProjects || {}).projects || []).filter(function (project) {
      return projectLinksFor(project).some(function (link) { return link.type === type && link.id === String(id); });
    });
    node.innerHTML = projects.length ? '<span>关联研究项目</span>' + projects.map(function (project) {
      return '<button type="button" data-linked-project="' + escapeHtml(project.id) + '">' + escapeHtml(project.title || '未命名项目') + ' ↗</button>';
    }).join('') : '';
    node.hidden = !projects.length;
  }
  function loadResearchProjects() {
    var optional = function (url) { return api(url).catch(function () { return { ok: false }; }); };
    return Promise.all([api('/api/research-projects'), optional('/api/references'), optional('/api/variable-library'), optional('/api/knowledge-base'), optional('/api/note-studio')]).then(function (results) {
      if (!results[0].ok) { toast(results[0].error || '请先登录后使用项目管理'); return; }
      state.researchProjects = Object.assign({ projects: [], trash: [] }, results[0].researchProjects || {});
      if (results[1].ok) state.referenceLibrary = results[1].referenceLibrary;
      if (results[2].ok) state.variableLibrary = results[2].variableLibrary;
      if (results[3].ok) state.knowledgeBase = results[3].knowledgeBase;
      if (results[4].ok) state.noteStudio = results[4].noteStudio;
      if (!state.projectId && state.researchProjects.projects[0]) state.projectId = state.researchProjects.projects[0].id;
  ...
  function addProjectLink() {
    ...
    if (!project || !projectLinkTypes[type] || !id || !projectLinkTargets(type).some(function (target) { return target.id === id; })) return;
    if (projectLinksFor(project).some(function (link) { return link.type === type && link.id === id; })) { toast('这条记录已关联'); return; }
    project.links = normalizedProjectLinks(project.links).concat({ type: type, id: id });
    renderProjectLinkControls(project);
    renderProjectSummary();
    saveResearchProject();
  }
  function removeProjectLink(type, id) {
    var project = activeResearchProject();
    if (!project) return;
    project.links = normalizedProjectLinks(project.links).filter(function (link) { return link.type !== type || link.id !== id; });
    renderProjectLinkControls(project);
    renderProjectSummary();
    saveResearchProject();
  }
  function openLinkedProject(id) {
    state.projectTrashOpen = false;
    state.projectCategoryFilter = 'all';
    state.projectId = id;
    switchPanel('research-projects');
  }
  function persistResearchProject(body, automatic) { return api('/api/research-projects', { method: 'POST', body: JSON.stringify(body) }).then(function (res) { if (!res.ok) throw new Error(res.error || '保存失败'); state.researchProjects = res.researchProjects; state.projectCategoryFilter = (body.category || '通用').trim() || '通用'; publishPaperProjectLinks(); if (!automatic) { renderResearchProjects(); toast('项目已同步保存'); } }); }
  ...
  function loadVariableLibrary() {
    return api('/api/variable-library').then(function (res) {
      ...
      return Promise.all([api('/api/references'), api('/api/research-projects')]).then(function (results) {
        if (results[0].ok) state.referenceLibrary = Object.assign({ items: [], trash: [], folders: [] }, results[0].referenceLibrary || {});
        if (results[1].ok) state.researchProjects = Object.assign({ projects: [], trash: [] }, results[1].researchProjects || {});
  ...
  function exportVariableCsv() {
    ...
    link.href = url;
    link.download = '变量库-' + new Date().toISOString().slice(0, 10) + '.csv';
  ...
  function parseVariableCsv(text) {
    ...
    if (quoted) throw new Error('CSV 文件中的双引号没有正确闭合');
    if (cell || row.length) { row.push(cell); rows.push(row); }
    return rows;
  }
  function importVariableCsvFile(event) {
    var file = event.target.files && event.target.files[0];
    if (!file) return;
    event.target.value = '';
    if (file.size > 5 * 1024 * 1024) { toast('CSV 文件不能超过 5 MB'); return; }
  ...
  function variableImportHistory() {
    var library = state.variableLibrary || {};
    if (Array.isArray(library.importHistory)) return library.importHistory;
  ...
  function closeVariableImportHistory() {
    $('#variableImportHistoryModal').hidden = true;
  }
  ...
  function undoVariableCsvImport(importId) {
    var history = variableImportHistory();
    var undo = history.filter(function (entry) { return !importId || String(entry.id || '') === String(importId); })[0];
  ...
  function normalizedVariableMeasureReferences(item) {
    if (!item) return [{ role: ['被解释变量'], source: '', measure: '', measures: [''], paper: '', papers: [''] }];
  ...
  function renderVariableLibrary() {
    ...
    if (importUndoButton) {
      var importHistory = variableImportHistory();
      importUndoButton.disabled = !importHistory.length || state.variableImportUndoPending;
      importUndoButton.title = importHistory.length ? '最近保留 ' + importHistory.length + ' 次 CSV 导入记录，最多保留 10 次' : '尚无可撤销的 CSV 导入记录';
      importUndoButton.textContent = 'CSV 导入历史' + (importHistory.length ? '（' + importHistory.length + '）' : '');
    }
  ...
  function newVariable(roleOverride) {
    if (state.variableCreating) return Promise.resolve();
    var requestedRoles = (Array.isArray(roleOverride) ? roleOverride : [roleOverride]).filter(function (role, index, all) { return variableRoles.indexOf(role) >= 0 && all.indexOf(role) === index; });
    if (!requestedRoles.length) requestedRoles = [variableRoles.indexOf(state.variableCategoryFilter) >= 0 ? state.variableCategoryFilter : '被解释变量'];
    var requestedRole = requestedRoles[0];
  ...
  function removeVariableMeasureOnly(groupIndex, measureIndex) {
    if (!state.variableId || state.variableTrashOpen || measureIndex <= 0) return;
  ...
  function addVariableReferenceOnly(index) {
    if (!state.variableId || state.variableTrashOpen) return;
    var entries = readVariableMeasureReferences(), entry = entries[index];
    if (!entry) return;
    entry.papers = (entry.papers || [entry.paper || '']).concat('');
    renderVariableMeasureReferences(entries, false);
    var next = $('[data-variable-paper="' + index + '"][data-variable-paper-index="' + (entry.papers.length - 1) + '"]', $('#variableMeasureReferences'));
    if (next) next.focus();
    queueVariableAutoSave();
  }
  function removeVariableReferenceOnly(groupIndex, paperIndex) {
    if (!state.variableId || state.variableTrashOpen || paperIndex <= 0) return;
    var entries = readVariableMeasureReferences(), entry = entries[groupIndex];
    if (!entry || !entry.papers || paperIndex >= entry.papers.length) return;
    entry.papers.splice(paperIndex, 1);
    entry.paper = entry.papers[0] || '';
    renderVariableMeasureReferences(entries, false);
    queueVariableAutoSave();
  }
  function removeVariableMeasureReference(index) {
    if (!state.variableId || state.variableTrashOpen) return;
    var entries = readVariableMeasureReferences();
    if (entries.length <= 1) entries[0] = { role: ['被解释变量'], source: '', measure: '', paper: '' };
  ...
  function readVariablePayload() {
    ...
    var active = activeVariable();
    var first = measureReferences[0] || { role: ['被解释变量'], source: '', measure: '', paper: '', papers: [''] };
    return { action: 'save', id: state.variableId, name: $('#variableName').value, role: first.role, paper: first.paper, definition: $('#variableDefinition').value, measure: first.measure, measureReferences: measureReferences, source: first.source, notes: active ? active.notes || '' : '' };
  }
  ...
  function createVariableReference(index, paperIndex, input) {
    ...
    api('/api/references', { method: 'POST', body: JSON.stringify({ action: 'create' }) }).then(function (created) {
      ...
      var referenceId = created.referenceLibrary.items[0].id;
      return api('/api/references', { method: 'POST', body: JSON.stringify({ action: 'save', id: referenceId, title: title, type: '期刊论文' }) });
    }).then(function (saved) {
    ...
    }).catch(function (error) { var paperInput = $('[data-variable-paper="' + index + '"][data-variable-paper-index="' + paperIndex + '"]', $('#variableMeasureReferences')); if (paperInput) delete paperInput.dataset.variablePaperCreated; toast(error.message || '新建参考文献失败'); });
  }
  ...
  // ===== 提示词库：模板变量、复制与回收站 =====
  function loadPromptLibrary() { return api('/api/prompt-library').then(function (res) { if (!res.ok) { toast(res.error || '请先登录后使用提示词库'); return; } state.promptLibrary = Object.assign({ prompts: [], trash: [], categories: ['通用'] }, res.promptLibrary || {}); if (!state.promptId && state.promptLibrary.prompts[0]) state.promptId = state.promptLibrary.prompts[0].id; renderPromptLibrary(); }); }
  ...
  function renderPromptFolderTree(categories, prompts) {
    var library = state.promptLibrary || { prompts: [], categories: ['通用'] };
    prompts = prompts || library.prompts || [];
    categories = categories || Array.from(new Set((library.categories || []).concat(prompts.map(function (prompt) { return (prompt.category || '通用').trim() || '通用'; }))));
    var list = $('#promptList'), query = String(state.promptQuery || '').trim().toLocaleLowerCase();
    var matches = function (prompt) {
      return [prompt.title, prompt.body, prompt.tags, prompt.category || '通用'].some(function (value) { return String(value || '').toLocaleLowerCase().indexOf(query) >= 0; });
    };
    var matchingPrompts = query ? prompts.filter(matches) : prompts;
    var visiblePrompts = matchingPrompts.length;
    var html = '<div class="prompt-category-heading"><span>文件夹</span><button type="button" data-prompt-category-create>＋ 新建分类</button></div>' +
      '<label class="prompt-library-search"><span aria-hidden="true">⌕</span><input id="promptSearch" type="search" placeholder="搜索标题、正文、标签或文件夹" aria-label="搜索提示词" value="' + escapeAttribute(state.promptQuery || '') + '"><kbd>Esc</kbd></label>' +
      (state.promptCategoryCreating ? '<form class="prompt-category-form" id="promptCategoryCreateForm"><input id="promptCategoryName" maxlength="40" placeholder="输入文件夹名称" aria-label="新分类名称" required><button type="submit">添加</button><button type="button" data-prompt-category-cancel>取消</button></form>' : '') +
      '<button type="button" class="prompt-folder-all' + (state.promptCategoryFilter === 'all' ? ' is-active' : '') + '" data-prompt-category="all"><span aria-hidden="true">▦</span><b>全部提示词</b><span class="prompt-folder-count">' + visiblePrompts + '</span></button>';
    var folderMarkup = categories.map(function (category, categoryIndex) {
      var categoryPrompts = prompts.filter(function (prompt) { return ((prompt.category || '通用').trim() || '通用') === category; });
      var categoryMatches = query && category.toLocaleLowerCase().indexOf(query) >= 0;
      var folderPrompts = query ? (categoryMatches ? categoryPrompts : categoryPrompts.filter(matches)) : categoryPrompts;
      if (query && !folderPrompts.length) return '';
      var collapsed = !query && state.promptCollapsedCategories[category] === true;
      var sortActions = query ? '' : '<span class="prompt-sort-actions"><button type="button" data-prompt-folder-move="' + escapeAttribute(category) + '" data-direction="-1" title="文件夹上移" aria-label="文件夹上移 ' + escapeAttribute(category) + '"' + (categoryIndex === 0 ? ' disabled' : '') + '>↑</button><button type="button" data-prompt-folder-move="' + escapeAttribute(category) + '" data-direction="1" title="文件夹下移" aria-label="文件夹下移 ' + escapeAttribute(category) + '"' + (categoryIndex === categories.length - 1 ? ' disabled' : '') + '>↓</button></span>';
      ...
      var folderActions = '<span class="prompt-folder-actions">' + sortActions + management + '</span>';
      var folderHeader = state.promptCategoryRenaming === category ? '<form class="prompt-folder-rename-form" id="promptCategoryRenameForm"><input id="promptFolderRenameName" maxlength="40" value="' + escapeAttribute(category) + '" aria-label="文件夹新名称" required><button type="submit">保存</button><button type="button" data-prompt-folder-rename-cancel>取消</button></form>' : '<div class="prompt-folder-row"><button type="button"' + (query ? '' : ' draggable="true"') + ' title="' + (query ? '点击选择文件夹' : '拖动以调整文件夹顺序；点击展开或折叠') + '" class="prompt-folder-toggle' + (state.promptCategoryFilter === category ? ' is-selected' : '') + '" data-prompt-folder-toggle="' + escapeAttribute(category) + '" aria-expanded="' + (!collapsed) + '"><span class="prompt-folder-chevron" aria-hidden="true">' + (collapsed ? '▸' : '▾') + '</span><span class="prompt-folder-icon" aria-hidden="true">' + (collapsed ? '📁' : '📂') + '</span><b>' + escapeHtml(category) + '</b><span class="prompt-folder-count">' + (query ? folderPrompts.length : categoryPrompts.length) + '</span></button>' + folderActions + (query ? '' : '<button type="button" class="prompt-folder-add" data-prompt-folder-new="' + escapeAttribute(category) + '" title="在此文件夹中新建提示词" aria-label="在 ' + escapeAttribute(category) + ' 中新建提示词">＋</button>') + '</div>';
      var promptRows = folderPrompts.length ? folderPrompts.map(function (prompt, promptIndex) {
        var itemActions = query ? '' : '<span class="prompt-sort-actions"><button type="button" data-prompt-item-move="' + prompt.id + '" data-direction="-1" title="提示词上移" aria-label="提示词上移：' + escapeAttribute(prompt.title || '未命名提示词') + '"' + (promptIndex === 0 ? ' disabled' : '') + '>↑</button><button type="button" data-prompt-item-move="' + prompt.id + '" data-direction="1" title="提示词下移" aria-label="提示词下移：' + escapeAttribute(prompt.title || '未命名提示词') + '"' + (promptIndex === folderPrompts.length - 1 ? ' disabled' : '') + '>↓</button></span>';
        return '<div class="prompt-file-row"><button type="button"' + (query ? '' : ' draggable="true"') + ' title="' + escapeAttribute(prompt.title || '未命名提示词') + '" class="prompt-list-item' + (Number(prompt.id) === Number(state.promptId) ? ' is-active' : '') + '" data-prompt-id="' + prompt.id + '"><b>' + escapeHtml(prompt.title || '未命名提示词') + '</b><span>' + escapeHtml(prompt.updated || '') + '</span></button>' + itemActions + '</div>';
      }).join('') : '<button type="button" class="prompt-folder-empty" data-prompt-folder-new="' + escapeAttribute(category) + '">此文件夹为空，点击 ＋ 新建</button>';
      return '<section class="prompt-folder">' + folderHeader + '<div class="prompt-folder-contents"' + (collapsed ? ' hidden' : '') + '>' + promptRows + '</div></section>';
    }).join('');
    html += '<div class="prompt-folder-tree">' + (folderMarkup || (query ? '<div class="prompt-search-empty">没有找到匹配的提示词</div>' : '')) + '</div>';
    list.innerHTML = html;
  }
  function renderPromptLibrary() {
    var library = state.promptLibrary || { prompts: [], trash: [], categories: ['通用'] }, prompts = library.prompts || [], trash = library.trash || [], list = $('#promptList');
    ...
    renderPromptFolderTree(categories, prompts);
  ...
  function createPromptCategory(value) {
    ...
    var library = state.promptLibrary || { prompts: [], trash: [], categories: [] };
    var categories = Array.from(new Set((library.categories || []).concat((library.prompts || []).map(function (item) { return (item.category || '通用').trim(); }), ['通用']).filter(Boolean)));
    var existing = categories.filter(function (category) { return category.toLocaleLowerCase() === name.toLocaleLowerCase(); })[0];
    var category = existing || name;
    var selectedPromptId = state.promptId;
    var request = existing ? Promise.resolve({ ok: true, promptLibrary: library }) : api('/api/prompt-library', { method: 'POST', body: JSON.stringify({ action: 'add-category', category: name }) });
    request.then(function (res) {
      if (!res.ok) { toast(res.error || '创建分类失败'); return; }
      state.promptLibrary = res.promptLibrary;
      state.promptLibrary.categories = Array.from(new Set((state.promptLibrary.categories || []).concat(category)));
      state.promptCategoryFilter = category;
      state.promptQuery = '';
      state.promptCollapsedCategories[category] = false;
      state.promptCategoryCreating = false;
      state.promptId = selectedPromptId;
      var selectedPrompt = (state.promptLibrary.prompts || []).filter(function (item) { return String(item.id) === String(selectedPromptId); })[0];
      if (selectedPrompt) selectedPrompt.category = category;
      renderPromptLibrary();
      if (selectedPrompt) queuePromptAutoSave();
      toast(existing ? '已切换到已有分类' : '分类已创建');
    }).catch(function () { toast('创建分类失败，请检查网络'); });
  }
  function renamePromptCategory(oldName, value) {
    var name = String(value || '').trim().slice(0, 40);
    if (!oldName || !name) { toast('文件夹名称不能为空'); return; }
    if (name === oldName) { state.promptCategoryRenaming = null; renderPromptLibrary(); return; }
    api('/api/prompt-library', { method: 'POST', body: JSON.stringify({ action: 'rename-category', category: oldName, newCategory: name, currentPrompt: state.promptId ? readPromptPayload() : null }) }).then(function (res) {
      if (!res.ok) { toast(res.error || '重命名文件夹失败'); return; }
      state.promptLibrary = res.promptLibrary;
      if (state.promptCategoryFilter === oldName) state.promptCategoryFilter = name;
      state.promptQuery = '';
      if (state.promptCollapsedCategories[oldName] !== undefined) state.promptCollapsedCategories[name] = state.promptCollapsedCategories[oldName];
      delete state.promptCollapsedCategories[oldName];
      state.promptCategoryRenaming = null;
      renderPromptLibrary();
      toast('文件夹已重命名');
    }).catch(function () { toast('重命名文件夹失败，请检查网络'); });
  }
  function deletePromptCategory(category) {
    if (!category) return;
    var count = (state.promptLibrary.prompts || []).filter(function (prompt) { return ((prompt.category || '通用').trim() || '通用') === category; }).length;
    var allCategories = Array.from(new Set((state.promptLibrary.categories || []).concat((state.promptLibrary.prompts || []).map(function (prompt) { return (prompt.category || '通用').trim() || '通用'; }))));
    var destinations = allCategories.filter(function (item) { return item !== category; }).sort(function (left, right) { if (left === '通用') return -1; if (right === '通用') return 1; return left.localeCompare(right, 'zh-CN'); });
    var destination = category === '通用' ? (destinations[0] || '未分类') : '通用';
    if (!confirm('确定删除“' + category + '”文件夹吗？其中的 ' + count + ' 条提示词会移入“' + destination + '”，内容不会删除。')) return;
    api('/api/prompt-library', { method: 'POST', body: JSON.stringify({ action: 'delete-category', category: category, destinationCategory: destination, currentPrompt: state.promptId ? readPromptPayload() : null }) }).then(function (res) {
      if (!res.ok) { toast(res.error || '删除文件夹失败'); return; }
      state.promptLibrary = res.promptLibrary;
      if (state.promptCategoryFilter === category) state.promptCategoryFilter = destination;
      delete state.promptCollapsedCategories[category];
      renderPromptLibrary();
  ...
  function shiftPromptFolder(category, direction) {
    var categories = promptFolderOrder();
    var sourceIndex = categories.indexOf(category), targetIndex = sourceIndex + direction;
    if (sourceIndex < 0 || targetIndex < 0 || targetIndex >= categories.length) return;
    categories.splice(sourceIndex, 1);
    categories.splice(targetIndex, 0, category);
    persistPromptLibraryOrder(categories, (state.promptLibrary.prompts || []).slice());
  }
  function reorderPromptItem(sourceId, targetId, afterTarget) {
    ...
    prompts.splice(targetIndex + (afterTarget ? 1 : 0), 0, source);
  ...
  function shiftPromptItem(promptId, direction) {
    var prompts = state.promptLibrary.prompts || [];
    var prompt = prompts.filter(function (item) { return String(item.id) === String(promptId); })[0];
    if (!prompt) return;
    var sameFolder = prompts.filter(function (item) { return (item.category || '通用').trim() === (prompt.category || '通用').trim(); });
    var sourceIndex = sameFolder.findIndex(function (item) { return String(item.id) === String(promptId); });
    var target = sameFolder[sourceIndex + direction];
    if (!target) return;
    reorderPromptItem(promptId, target.id, direction > 0);
  }
  ...
  function loadNoteStudio() {
    return Promise.all([api('/api/note-studio'), api('/api/research-projects')]).then(function (results) {
      if (!results[0].ok) { toast(results[0].error || '请先登录后使用笔记'); return; }
      state.noteStudio = Object.assign({ notes: [], trash: [] }, results[0].noteStudio || {});
      if (results[1].ok) state.researchProjects = Object.assign({ projects: [], trash: [] }, results[1].researchProjects || {});
  ...
  function renderNoteStudio() {
    ...
    if (state.noteTrashOpen) {
      ...
      title.value = ''; title.disabled = true; editor.value = ''; editor.disabled = true; style.disabled = true; remove.hidden = true; $('#notePreview').innerHTML = '<div class="note-empty">可在左侧恢复误删笔记。</div>'; renderProjectBacklinks('note', null, $('.note-editor-card')); return;
    }
    ...
    renderNotePreview();
    renderProjectBacklinks('note', active && active.id, $('.note-editor-card'));
  }
  ...
  function loadKnowledgeBase() {
    return Promise.all([api('/api/knowledge-base'), api('/api/research-projects')]).then(function (results) {
      if (!results[0].ok) { toast(results[0].error || '请先登录后使用知识库'); return; }
      state.knowledgeBase = results[0].knowledgeBase || { folders: [], docs: [] };
      if (results[1].ok) state.researchProjects = Object.assign({ projects: [], trash: [] }, results[1].researchProjects || {});
      renderKnowledgeBase();
    });
  }
