...
(function () {
  ...
  window.fetch = async function (input, options) {
    ...
    try {
      ...
      if (path === '/api/variable-library') {
        ...
        if (body.action === 'import' && Array.isArray(body.items)) {
          ...
          var importedVariables = body.items.slice(0, 500).map(function (source) {
            ...
            var entries = (Array.isArray(source.measureReferences) ? source.measureReferences : []).slice(0, 100).map(function (entry) { entry = entry || {}; var measures = Array.isArray(entry.measures) ? entry.measures.slice(0, 30).map(function (value) { return String(value || '').slice(0, 20000); }) : [String(entry.measure || '').slice(0, 20000)]; if (!measures.length) measures = ['']; return { role: [role], source: String(entry.source || '').slice(0, 20000), measure: measures[0], measures: measures, paper: String(entry.paper || '').trim().slice(0, 500) }; });
        ...
        if (body.action === 'duplicate') {
          ...
          if (String(currentVariable.id) === String(sourceVariable.id)) {
            ...
            if (Array.isArray(currentVariable.measureReferences)) sourceVariable.measureReferences = currentVariable.measureReferences.slice(0, 100).map(function (entry) { entry = entry || {}; var entryRoles = (Array.isArray(entry.role) ? entry.role : [entry.role]).filter(function (role, index, all) { return variableRoles.indexOf(role) >= 0 && all.indexOf(role) === index; }); var measures = Array.isArray(entry.measures) ? entry.measures.slice(0, 30).map(function (value) { return String(value || '').slice(0, 20000); }) : [String(entry.measure || '').slice(0, 20000)]; if (!measures.length) measures = ['']; return { role: entryRoles.length ? entryRoles : ['其他'], source: String(entry.source || '').slice(0, 20000), measure: measures[0], measures: measures, paper: String(entry.paper || '').trim().slice(0, 500) }; });
        ...
        if (body.action === 'save') variableLibrary.items.forEach(function (item) {
          ...
          if (Array.isArray(body.measureReferences)) item.measureReferences = body.measureReferences.slice(0, 100).map(function (entry) { entry = entry || {}; var entryRoles = (Array.isArray(entry.role) ? entry.role : [entry.role]).filter(function (role, index, all) { return variableRoles.indexOf(role) >= 0 && all.indexOf(role) === index; }); var measures = Array.isArray(entry.measures) ? entry.measures.slice(0, 30).map(function (value) { return String(value || '').slice(0, 20000); }) : [String(entry.measure || '').slice(0, 20000)]; if (!measures.length) measures = ['']; return { role: entryRoles.length ? entryRoles : ['其他'], source: String(entry.source || '').slice(0, 20000), measure: measures[0], measures: measures, paper: String(entry.paper || '').trim().slice(0, 500) }; });
      ...
      if (path === '/api/references') { var references = data.referenceLibrary || (data.referenceLibrary = { items: [], trash: [] }); references.items = Array.isArray(references.items) ? references.items : []; references.trash = Array.isArray(references.trash) ? references.trash : []; if (method === 'GET') return response({ ok: true, referenceLibrary: references }); if (body.action === 'create') references.items.unshift({ id: nowId(), title: '未命名文献', authors: '', year: '', type: '期刊论文', source: '', locator: '', doi: '', url: '', tags: '', projectId: '', knowledgeDocId: '', notes: '', updated: nowText() }); if (body.action === 'save') references.items.forEach(function (item) { if (String(item.id) === String(body.id)) { item.title = String(body.title || '未命名文献').trim(); item.authors = String(body.authors || '').trim(); item.year = String(body.year || '').replace(/[^0-9]/g, '').slice(0, 4); item.type = ['期刊论文', '书籍', '会议论文', '报告', '网页'].indexOf(body.type) >= 0 ? body.type : '期刊论文'; item.source = String(body.source || '').trim(); item.locator = String(body.locator || '').trim(); item.doi = String(body.doi || '').trim().replace(/^https?:\/\/(?:dx\.)?doi\.org\//i, ''); item.url = String(body.url || '').trim(); item.tags = String(body.tags || '').trim(); item.projectId = String(body.projectId || ''); item.knowledgeDocId = String(body.knowledgeDocId || ''); item.notes = String(body.notes || ''); item.updated = nowText(); } }); if (body.action === 'trash') { var removedReference = references.items.filter(function (item) { return String(item.id) === String(body.id); })[0]; if (removedReference) { references.trash.unshift({ id: nowId(), item: removedReference, deletedAt: nowText() }); references.items = references.items.filter(function (item) { return String(item.id) !== String(body.id); }); } } if (body.action === 'restore') { var restoredReference = references.trash.filter(function (entry) { return String(entry.id) === String(body.id); })[0]; if (restoredReference && restoredReference.item) { references.items.unshift(restoredReference.item); references.trash = references.trash.filter(function (entry) { return String(entry.id) !== String(body.id); }); } } if (body.action === 'purge') references.trash = references.trash.filter(function (entry) { return String(entry.id) !== String(body.id); }); if (body.action === 'purge-all') references.trash = []; await saveWorkspace(data, dataRevision); return response({ ok: true, referenceLibrary: references }); }
